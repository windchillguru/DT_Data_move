...
import ext.dataMove.runnable.ExportECRunnable;
...
public class ExportEC implements RemoteAccess {
	...
	public static void process(ArgsInfo argsInfo) throws Exception {
		...
		try {
			...
			processThread(containerList, 0, ts1, ts2, argsInfo.getDetail());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void processThread(List containerList, int start, Timestamp ts1, Timestamp ts2, String type) {
		try {
			...
			for (int i = start; i < num; i++) {
				...
				ExportECRunnable edr = new ExportECRunnable();
			...
			if (num < containerList.size()) {
				start = num;
				processThread(containerList, start, ts1, ts2, type);
			} else {
				return;
			}
		} catch (Exception e) {
