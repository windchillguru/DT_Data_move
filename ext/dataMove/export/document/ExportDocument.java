package ext.dataMove.export.document;
...
import ext.dataMove.runnable.ExportDocRunnable;
import ext.dataMove.runnable.ExportPartRunnable;
...
public class ExportDocument implements RemoteAccess{
	/**
	 * @param rootNumber
	 * @param filePath
	 * @throws Exception
	...
	public static void processThread(List containerList, int start, Timestamp ts1, Timestamp ts2) {
		try {
			...
			for (int i = start; i < num; i++) {
				...
				// log.toString());
				ExportDocRunnable edr = new ExportDocRunnable();
